...
class ToDoModel:
    ...
    def get_by_id(self, _id):
        where_clause = "AND id= \"" + str({_id}) +" \""
        return self.list_items(where_clause)

    def create(self, params):
        title = str(params.get("Title"))
        desc = str(params.get("Description"))
        DueDate = str(params.get("DueDate"))
        UserId = str(params.get("UserId"))

        query = " insert into " + str(self.TABLENAME) + " (Title, Description, DueDate, UserId)  " + " values ( \" " + title + "\", \"" + desc + "\", \"" + DueDate + "\", \"" + UserId + "\") "
        print(query)
